...
namespace MMM{
	...
	class ConcurrentQueue_2{
	private:
		using atomic_uint32 = std::atomic<unsigned>;

		// array of data
		// data is pushed onto the back
		// data is popped from the front
		_T* data_;

		// maximum number of elements the queue can currently have
		atomic_uint32 a_capacity_;

		// current number of not-popped elements the queue has
		atomic_uint32 a_size_;

		// current index of the head data starts from 0
		atomic_uint32 a_head_;

		// current index of the tail place to put new elements
		atomic_uint32 a_tail_;

		// --- sentinel variables ---
		// variables used to sychronize
		// data access and manipution
		// across multiple threads

		// number of threads currently trying to push data onto the queue
		atomic_uint32 a_pushing_;

		// number of threads currently trying to pop data from the queue
		atomic_uint32 a_popping_;

		// true when the queue is resizing the data array
	...
	public:
		...
		ConcurrentQueue_2(unsigned initial_capacity)
			:
		...
		void PushBack(_T& data){
			...
			// increment the counter for the number of threads currently
			// adding elements onto the queue
			a_pushing_.fetch_add(1u, std::memory_order_acq_rel);

			// atomic increment the tail of the array
			// store the index of the tail before the incremenet
			auto tail = a_tail_.fetch_add(1u, std::memory_order_acq_rel);

			// atomic load the maximum capacity of the array
			auto capacity = a_capacity_.load(std::memory_order_acq_rel);

			// check if the array needs to be resized or updated
			if (tail >= capacity){

				// decrease the number of threads currently trying
				// to add an element to the queue
				a_pushing_.fetch_sub(1u, std::memory_order_acq_rel);
			...
			// add data to the secured place in the array
			data_[tail] = data;

			// increment the size
			// * number of not-popped elements
			a_size_.fetch_add(1u, std::memory_order_release);

			// decrement the number of threads currently pushing
			a_pushing_.fetch_sub(1u, std::memory_order_release);
	...
	private:
		void Resize(){
			...
			// load the current capacity of the queue
			...
			// ! one that is made from measurements
			...
			if (usage_ratio > MMM_MAGIC_RATIO){
				...
				// if _T is a pointer
				// loop through all already popped elements and delete them
				if (std::is_pointer<_T>::value){
					for (auto index = 0u; index < head; ++index){
						//delete data_[index];
					}
				}
				...
				if (std::is_pointer<_T>::value){
					for (auto index = 0u; index < head; ++index){
						//delete old_array[index];
					}
				}

				// copy the usable elements from the old array the beginning of the new array
